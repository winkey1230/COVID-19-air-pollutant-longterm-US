## 更改了forester函数两个地方：
# 横坐标保留为两位小数， 改accuracy
# 参数arrows 改为arrowss来避免与函数名重复
forester_update <- function (left_side_data, estimate, ci_low, ci_high, ci_sep = " to ", 
          right_side_data = NULL, estimate_precision = 1, ggplot_width = 30, 
          null_line_at = 0, file_path = file.path(tempdir(), paste0("forester_plot.", 
                                                                    render_as)), dpi = 600, display = TRUE, font_family = "mono", 
          estimate_col_name = "Estimate", stripe_colour = "#eff3f2", 
          background_colour = "white", x_scale_linear = TRUE, 
          xlim = NULL, xbreaks = NULL, nudge_y = 0, nudge_x = 1, nudge_height = 0, 
          nudge_width = 0, justify = 0, arrowss = FALSE, arrow_labels = c("Lower", 
                                                                         "Higher"), add_plot = NULL, add_plot_width = 1, 
          add_plot_gap = FALSE, point_sizes = 3, point_shapes = 16, 
          center_ggplot = NULL, lower_header_row = FALSE, render_as = "png",colors = "black") 
{ if (!length(justify) == 1) {
    justify <- c(justify[1:(length(justify) - 1)], 0, justify[length(justify)])
    justify <- matrix(justify, ncol = length(justify), nrow = nrow(left_side_data) + 
                        3, byrow = TRUE)
    justify <- as.vector(justify)
  }
  if (lower_header_row == FALSE) {
    theme <- gridExtra::ttheme_minimal(core = list(fg_params = list(hjust = justify, 
                                                                    x = (0.05 + (0.45/0.5) * justify), fontfamily = font_family), 
                                                   bg_params = list(fill = c(rep(c(stripe_colour, background_colour), 
                                                                                 length.out = nrow(left_side_data)), background_colour, 
                                                                             background_colour, background_colour))), colhead = list(fg_params = list(hjust = 0, 
                                                                                                                                                      x = 0.05, fontfamily = font_family), bg_params = list(fill = background_colour)))
  }
  else {
    theme <- gridExtra::ttheme_minimal(core = list(fg_params = list(hjust = justify, 
                                                                    x = (0.05 + (0.45/0.5) * justify), fontfamily = font_family), 
                                                   bg_params = list(fill = c(rep(c(background_colour, 
                                                                                   stripe_colour), length.out = nrow(left_side_data)), 
                                                                             background_colour, background_colour, background_colour))), 
                                       colhead = list(fg_params = list(hjust = 0, x = 0.05, 
                                                                       fontfamily = font_family, fill = background_colour), 
  ...
  if (is.null(right_side_data)) {
    ...
    right_side_data <- data.frame(Estimate = ifelse(tdata$estimate == 
                                                      " ", " ", paste0(tdata$estimate, " (", 
                                                                       tdata$ci_low, ci_sep, tdata$ci_high, ")")))
  ...
  white_column <- function(table, col) {
    ...
    for (i in ind) {
      table$grobs[i][[1]][["gp"]] <- grid::gpar(fill = background_colour, 
  ...
  h_adj <- dplyr::case_when(font_family == "mono" ~ 0.2, 
                            font_family == "serif" ~ 0.43, font_family == "sans" ~ 
                              0.37, TRUE ~ 0)
  h_adj <- nudge_y + h_adj
  slope_adj <- dplyr::case_when(font_family == "mono" ~ 
                                  -0.175, font_family == "serif" ~ -0.19, font_family == 
                                  "sans" ~ -0.16, TRUE ~ 0)
  ...
  if (!is.null(xlim)) {
    ...
    if (ra) {
      right_arrows <- dplyr::select(dplyr::filter(oob_arrows, 
                                                  ci_high > .data$x_high), start = estimate, end = .data$x_high, 
                                    y = .data$row_num)
    }
    if (la) {
      left_arrows <- dplyr::select(dplyr::filter(oob_arrows, 
                                                 ci_low < .data$x_low), start = estimate, end = .data$x_low, 
                                   y = .data$row_num)
    }
    if (ra && !la) {
      g_oob <- right_arrows
  ...
  center <- ggplot2::ggplot() + ggplot2::geom_point(data = gdata, 
                                                    ggplot2::aes(y = row_num, x = estimate, size = sizes,
                                                                 shape = shape), na.rm = TRUE,colour = colors,show.legend = F) + ggplot2::geom_errorbarh(data = gdata, 
                                                                                                                         ggplot2::aes(y = row_num, xmin = ci_low, xmax = ci_high), 
                                                                                                                         height = 0.25, na.rm = TRUE,show.legend = F,colour = colors) + ggplot2::theme_classic() + 
  ...
  if (x_scale_linear) {
    if (is.null(xbreaks)) {
      center <- center + ggplot2::scale_x_continuous(labels = scales::number_format(accuracy = 0.01), 
                                                     expand = c(0, 0))
    }
    else {
      center <- center + ggplot2::scale_x_continuous(labels = scales::number_format(accuracy = 0.01), 
    ...
    if (is.null(xbreaks)) {
      center <- center + ggplot2::scale_x_log10(labels = scales::number_format(accuracy = 0.01), 
                                                expand = c(0, 0))
    }
    else {
      center <- center + ggplot2::scale_x_log10(labels = scales::number_format(accuracy = 0.01), 
    ...
    arrows_plot <- ggplot2::ggplot() + ggplot2::geom_segment(data = arrow_df, 
                                                             ggplot2::aes(x = .data$xstart, xend = .data$xend, 
                                                                          y = .data$y, yend = .data$y), arrow = ggplot2::arrow(angle = 15, 
                                                                                                                               type = "closed", length = grid::unit(0.1, 
                                                                                                                                                                    "in"))) + ggplot2::geom_text(data = xlab_df, 
                                                                                                                                                                                                 ggplot2::aes(x = .data$x, y = .data$y, label = .data$text, 
                                                                                                                                                                                                              hjust = .data$hjust), family = font_family, size = 3) + 
      ggplot2::scale_y_continuous(expand = c(0, 0), limits = c(-0.5, 
                                                               1.75)) + ggplot2::scale_x_continuous(expand = c(0, 
  ...
  if (!(render_as == "rmarkdown")) {
    ggplot2::ggsave(dpi = dpi, height = png_height + nudge_height, 
                    width = png_width + nudge_width, units = "in", 
                    filename = file_path, device = render_as)
    if (display == TRUE) {
